...
def get_model_predictions(WEKA_PATH, RESULTS_PATH, MODELS, CLASSIFIER, ensembl_votes, ORIGINAL_IDENTIFIERS, SEQUENCES):
    ...
    for model in MODELS:
        #--------------------------------------------------------------
        ParamList = ['java', '-cp', WEKA_PATH, CLASSIFIER, '-l', model, '-T', pjoin(RESULTS_PATH, 'weka.arff'), '-p', 'first-last']

        with open(pjoin(RESULTS_PATH, 'Predictions.txt'), 'wb') as out:
            try:
        ...
        file_input = pjoin(RESULTS_PATH, 'Predictions.txt')
